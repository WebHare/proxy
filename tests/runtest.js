...
async function test()
{
  ...
  for (let i = 0; i < 600; ++i)
  {
    ...
    try
    {
      await fetch(`http://${config.PROXYIP}:${config.PORT_80}/`,
  ...
  let response = await fetch(`http://${config.PROXYIP}:${config.PORT_80}/`,
    { headers : { host : 'admin.example.com' }
    });
  ...
  response = await fetch(`http://${config.PROXYIP}:${config.PORT_80}/admin/rpc`,
    { headers : { host : 'admin.example.com'
  ...
  if (response.status != 200)
    throw new Error(`Unexpected status return code ${response.status}`);
  ...
  for (let i = 0; i < 100; ++i)
  {
    ...
    response = await fetch(`http://${config.PROXYIP}:${config.PORT_80}/normalrequest`,
  ...
  response = await fetch(`https://${config.PROXYIP}:${config.PORT_443}/normalrequest`,
  ...
  response = await fetch(`http://${config.PROXYIP}:${config.PORT_80}/normalrequest/addremoteip`,
    { headers : { host : 'test.example.com' }
    , method: 'GET'
    });

  let expect_servertiming = `remoteip;desc="${config.EXPECTSEENCONNECTIP || config.CONNECTIP}"`;
  if (response.headers.get("server-timing") != expect_servertiming)
